...
void VehicleBodyLightsFogRearPublisher::subscribeToAllChanges(IVehicleBodyLightsFogRearSubscriber& subscriber)
{
    ...
    {
        m_allChangesSubscribers.push_back(std::reference_wrapper<IVehicleBodyLightsFogRearSubscriber>(subscriber));
    }
}

void VehicleBodyLightsFogRearPublisher::unsubscribeFromAllChanges(IVehicleBodyLightsFogRearSubscriber& subscriber)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_allChangesSubscribersMutex);
    auto found = std::find_if(m_allChangesSubscribers.begin(), m_allChangesSubscribers.end(),
                        [&subscriber](const auto element){return &(element.get()) == &subscriber;});
...
long VehicleBodyLightsFogRearPublisher::subscribeToIsOnChanged(VehicleBodyLightsFogRearIsOnPropertyCb callback)
{
...
void VehicleBodyLightsFogRearPublisher::unsubscribeFromIsOnChanged(long handleId)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_isOnCallbacksMutex);
    m_isOnCallbacks.erase(handleId);
}

void VehicleBodyLightsFogRearPublisher::publishIsOnChanged(bool IsOn) const
{
...
long VehicleBodyLightsFogRearPublisher::subscribeToIsDefectChanged(VehicleBodyLightsFogRearIsDefectPropertyCb callback)
{
...
void VehicleBodyLightsFogRearPublisher::unsubscribeFromIsDefectChanged(long handleId)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_isDefectCallbacksMutex);
    m_isDefectCallbacks.erase(handleId);
}

void VehicleBodyLightsFogRearPublisher::publishIsDefectChanged(bool IsDefect) const
{
    std::shared_lock<std::shared_timed_mutex> allChangesSubscribersLock(m_allChangesSubscribersMutex);
    const auto allChangesSubscribers = m_allChangesSubscribers;
    allChangesSubscribersLock.unlock();
