...
void VehicleBodyLightsFogFrontPublisher::subscribeToAllChanges(IVehicleBodyLightsFogFrontSubscriber& subscriber)
{
    ...
    {
        m_allChangesSubscribers.push_back(std::reference_wrapper<IVehicleBodyLightsFogFrontSubscriber>(subscriber));
    }
}

void VehicleBodyLightsFogFrontPublisher::unsubscribeFromAllChanges(IVehicleBodyLightsFogFrontSubscriber& subscriber)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_allChangesSubscribersMutex);
    auto found = std::find_if(m_allChangesSubscribers.begin(), m_allChangesSubscribers.end(),
                        [&subscriber](const auto element){return &(element.get()) == &subscriber;});
...
long VehicleBodyLightsFogFrontPublisher::subscribeToIsOnChanged(VehicleBodyLightsFogFrontIsOnPropertyCb callback)
{
...
void VehicleBodyLightsFogFrontPublisher::unsubscribeFromIsOnChanged(long handleId)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_isOnCallbacksMutex);
    m_isOnCallbacks.erase(handleId);
}

void VehicleBodyLightsFogFrontPublisher::publishIsOnChanged(bool IsOn) const
{
...
long VehicleBodyLightsFogFrontPublisher::subscribeToIsDefectChanged(VehicleBodyLightsFogFrontIsDefectPropertyCb callback)
{
...
void VehicleBodyLightsFogFrontPublisher::unsubscribeFromIsDefectChanged(long handleId)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_isDefectCallbacksMutex);
    m_isDefectCallbacks.erase(handleId);
}

void VehicleBodyLightsFogFrontPublisher::publishIsDefectChanged(bool IsDefect) const
{
    std::shared_lock<std::shared_timed_mutex> allChangesSubscribersLock(m_allChangesSubscribersMutex);
    const auto allChangesSubscribers = m_allChangesSubscribers;
    allChangesSubscribersLock.unlock();
