...
void VehicleBodyLightsBackupPublisher::subscribeToAllChanges(IVehicleBodyLightsBackupSubscriber& subscriber)
{
    ...
    {
        m_allChangesSubscribers.push_back(std::reference_wrapper<IVehicleBodyLightsBackupSubscriber>(subscriber));
    }
}

void VehicleBodyLightsBackupPublisher::unsubscribeFromAllChanges(IVehicleBodyLightsBackupSubscriber& subscriber)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_allChangesSubscribersMutex);
    auto found = std::find_if(m_allChangesSubscribers.begin(), m_allChangesSubscribers.end(),
                        [&subscriber](const auto element){return &(element.get()) == &subscriber;});
...
long VehicleBodyLightsBackupPublisher::subscribeToIsOnChanged(VehicleBodyLightsBackupIsOnPropertyCb callback)
{
...
void VehicleBodyLightsBackupPublisher::unsubscribeFromIsOnChanged(long handleId)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_isOnCallbacksMutex);
    m_isOnCallbacks.erase(handleId);
}

void VehicleBodyLightsBackupPublisher::publishIsOnChanged(bool IsOn) const
{
...
long VehicleBodyLightsBackupPublisher::subscribeToIsDefectChanged(VehicleBodyLightsBackupIsDefectPropertyCb callback)
{
...
void VehicleBodyLightsBackupPublisher::unsubscribeFromIsDefectChanged(long handleId)
{
    std::unique_lock<std::shared_timed_mutex> lock(m_isDefectCallbacksMutex);
    m_isDefectCallbacks.erase(handleId);
}

void VehicleBodyLightsBackupPublisher::publishIsDefectChanged(bool IsDefect) const
{
    std::shared_lock<std::shared_timed_mutex> allChangesSubscribersLock(m_allChangesSubscribersMutex);
    const auto allChangesSubscribers = m_allChangesSubscribers;
    allChangesSubscribersLock.unlock();
